...
#   Timeout()
#       Timeout a function
#       src: http://programming-guides.com/python/timeout-a-function
###################################################
...
def timeout():
    def timeout_function(f):
        def f2(*args):
            ...
            signal.alarm(options.timeout)  # triger in timeout_time seconds
            try:
                retval = f()
...
def check_snmp_extend():
    ...
    for snmp_extend in output_table.items():
        ...
        if options.output_complete_summary:
            add_summary("%s=%s, " % (snmp_extend[0],
                                     snmp_extend[1]["Summary"]))
        else:
            add_summary("%s=%s, " % (snmp_extend[0],
                        state_nagios_text[snmp_extend[1]["Result"]]))
        ...
        if snmp_extend[1]["LongOutput"] != "" and options.output_longoutput:
            add_long_output("%s=%s, " % (snmp_extend[0],
                            snmp_extend[1]["LongOutput"]))
...
def parse_options():
    ...
    Call this plugin like this to check this snmp extend :
        check_snmp_extend.py
            --host localhost
            --snmp-version 2c
            --community public
            --extend-name yesterday
    ...
    parser.add_option("-l", "--seclevel", dest="snmp_seclevel",
                      help="SNMP Seclevel "
                            "(noAuthNoPriv|authNoPriv|authPriv,v3 only)")
...
def end():
    ...
    if options.extend_name == 'ALL':
        message = "%s - ok objects: %s, not ok objects: %s - %s" % (
